...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET' && response.ok && type.includes('text/html')) ctx.waitUntil(record(request, env));
    return response;
  }
};
...
async function record(request, env) {
  ...
  const isBot = /bot|crawler|spider|slurp|facebookexternalhit|whatsapp|telegrambot|discordbot|slackbot|pingdom|uptimerobot|ahrefs|semrush|mj12bot|dotbot|petalbot|bytespider|applebot|preview|headless/i.test(agent);
  const browser = isBot ? 'Bot' : /mobile|android|iphone|ipad/i.test(agent) ? 'Mobile' : 'Desktop';
...
async function summary(url, env) {
  ...
  const [totals, referrers, pages, countries, browsers, daily] = await Promise.all([
    ...
    bind('SELECT substr(seen_at,1,10) day, COUNT(*) count FROM events WHERE seen_at BETWEEN ? AND ? GROUP BY day ORDER BY day').all()
  ]);
  return json({totals, referrers:referrers.results, pages:pages.results, countries:countries.results, browsers:browsers.results, daily:daily.results});
}
